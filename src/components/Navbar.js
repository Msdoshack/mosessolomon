...
const Navbar = () => {
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      // Check if the click is outside of the popup and also not on the hamburger button itself
      if (
        popupRef.current &&
  ...
  return (
    <header className=" font-medium  border-dark/10  dark:border-light/10  border-b-2 sticky top-0 z-40 bg-light dark:bg-darkblue">
      <div className="flex w-full px-32 py-6 lg:px-16 md:px-12 sm:ps-4 items-center justify-between relative">
        <button
          ref={buttonRef}
          className="hidden md:flex flex-col justify-center items-center  z-30"
        ...
        {isOpen && (
          <motion.div
            ref={popupRef}
            className="hidden min-w-[70vw] md:flex flex-col items-center justify-between fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2  z-30 dark:bg-light/75 bg-dark/90 backdrop-blur-md rounded-lg py-32"
